...
function XYZ_original = OF_grid_filterer_FirstSecondThird_v200b1(XYZ_original , max_dist , Nplanes_min )
...
while sumElim~=0 || Nff~=0
    
    k   = k + 1 ;
    N1  = length(XYZ_original_tmp) ;
    ...
    disp('Filtering the grid for added cell centroids...')
    XYZ_original_tmp = grid_filterer_first_v100( XYZ_original_tmp , Nplanes_min ) ;
    
    %% see how many points were filtered
    N2 = length(XYZ_original_tmp) ;
    Nff = N1-N2 ;
